...
관리자모드 별도 산출물입니다.
진입 파일은 `/tdwfpub/admin/admin-login.html`이며, 로그인 후 메인 화면은 `/tdwfpub/admin/admin-main.html`입니다.

## 기본 구조

- 공통 JS: `/tdwfpub/admin/js/admin.js`
- include 로드: `/tdwfpub/admin/js/include.js`
- form 관련 JS: `/tdwfpub/admin/js/form.js` (퍼블리싱용으로 임의작업 한 내용이니 전부 제거 및 변경하셔도 무방합니다.)
...
- 방문 페이지 탭은 `/tdwfpub/admin/js/admin.js`에서 `localStorage`에 저장합니다.
