// 사이트 구분: body[data-site] 값으로 admin/cooperation 사이트가 이 파일을 함께 사용합니다.
const adminSiteName = document.body.dataset.site || 'admin';
const adminMainPageName = `${adminSiteName}-main.html`;
const adminTabStorageKey = `twf-${adminSiteName}-open-tabs`;
const adminTabMaxCount = 8;
const focusableSelector =
  'a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])';
// 팝업별 여는 버튼: 팝업 안에서 다른 팝업을 여는 경우까지 포커스를 되돌립니다.
const activePopupButtons = new WeakMap();
...
const saveAdminTab = (href, label, pageName = '') => {
  ...
  if (targetUrl.pathname.endsWith(`/${adminMainPageName}`)) return;
...
/**
 * 테이블 행 클릭 시 행 안의 체크박스 선택/해제
 */
const bindTableRowChecks = () => {
  const rows = document.querySelectorAll('[data-row-check]:not([data-row-check-bound])');
  ...
  rows.forEach((row) => {
    ...
    row.dataset.rowCheckBound = 'true';
    row.addEventListener('click', (event) => {
      if (event.target.closest('label, a, button, input, select, textarea')) return;
 ...
 * 팝업 단계 이동 이벤트 연결
 */
const bindPopupSteps = () => {
  document.querySelectorAll('[data-popup-steps]:not([data-popup-steps-bound])').forEach((group) => {
    const steps = Array.from(group.querySelectorAll('[data-popup-step]'));
    const root = group.closest('[data-popup]') || document;
    const prevButton = root.querySelector('[data-step-prev]');
    const nextButton = root.querySelector('[data-step-next]');
    const submitButton = root.querySelector('[data-step-submit]');

    if (!steps.length) return;

    group.dataset.popupStepsBound = 'true';
    ...
    const move = (step) => {
      current = Math.min(Math.max(current + step, 0), steps.length - 1);
      render();
...
const closePopup = (popup) => {
  ...
  // 남아 있는 팝업 기준으로 스크롤 잠금 갱신
  document.documentElement.classList.toggle('popup-open', Boolean(document.querySelector('[data-popup].is-open')));

  const openButton = activePopupButtons.get(popup);

  activePopupButtons.delete(popup);
...
const openPopup = (popup, button) => {
  ...
  activePopupButtons.set(popup, button);
...
const bindPopup = () => {
  document.querySelectorAll('[data-popup-open]:not([data-popup-open-bound])').forEach((button) => {
    const open = (event) => {
      if (!button.matches('a, button') && event.target.closest('a, button, label')) return;
    ...
    // 링크·버튼이 아닌 요소(목록 행 등)는 Enter 키로도 열 수 있게 합니다.
    if (button.matches('a, button')) return;
 ...
 * 헤더 자주 찾는 메뉴 드롭다운 연결
 */
const bindFavoriteMenu = () => {
  const favoriteMenu = document.querySelector('[data-favorite-menu]');
  if (!favoriteMenu || favoriteMenu.dataset.favoriteMenuBound) return;

  const toggle = favoriteMenu.querySelector('[data-favorite-menu-toggle]');
  const panel = favoriteMenu.querySelector('[data-favorite-menu-panel]');
  if (!toggle || !panel) return;

  favoriteMenu.dataset.favoriteMenuBound = 'true';
  ...
  document.addEventListener('click', (event) => {
    if (!favoriteMenu.contains(event.target)) closeFavoriteMenu();
  });
